...
def test_IKM_to_lamport_SK():
    ...
    IKM = test_vector['intermediate_SK'].to_bytes(32, 'big')
    lamport_0 = IKM_to_lamport_SK(IKM=IKM, salt=salt)
    not_IKM = flip_bits(test_vector['intermediate_SK']).to_bytes(32, 'big')
...
def test_HKDF_mod_r():
    test_0 = (bytes.fromhex(test_vector['seed']), test_vector['intermediate_SK'])
...
def test_derive_child_SK():
    test_parent_SK = test_vector['intermediate_SK']
